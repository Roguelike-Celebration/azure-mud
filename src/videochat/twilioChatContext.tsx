...
export const TwilioChatContextProvider = (props: {
  ...
  const [remoteParticipants, setRemoteParticipants] =
    useState<Map<String, Twilio.Participant>>();

  const [localVideoTrack, setLocalVideoTrack] =
    useState<Twilio.LocalVideoTrack>();
  const [localAudioTrack, setLocalAudioTrack] =
    useState<Twilio.LocalAudioTrack>();
  ...
  const fetchLocalVideoTrack = async () => {
    ...
    const options: Twilio.CreateLocalTrackOptions = {
      // TODO: Shrink size if mobile
      height: 720,
      frameRate: 24,
      width: 1280,
    };
  ...
  async function prepareForMediaChat() {
    if (token) return;
    return fetchTwilioToken().then((token) => {
      setToken(token);
    });
  }
  ...
  async function joinCall(roomId: string) {
    ...
    try {
      ...
      console.log(
        "[TWILIO] Attached participant count:",
        room.participants.size
      );

      setLocalStreamView(
        <ParticipantTracks participant={room.localParticipant} />
      );

...
This project is set up to automatically deploy to GitHub Pages when code is merged into master (see "Deployment and CI/CD via GitHub Actions" below for how to finish configuring that for your fork). If you want to host your frontend elsewhere, that's totally fine too! You can serve the asset bundle generated by `npm run build` anywhere, although it will need to be served over SSL/HTTPS.
...
By default, every time code is merged into the `main` branch in this repo, both the frontend and backend are deployed. It's very little work to configure this same behavior on your GitHub fork of this project; see "Deployment and CI/CD via GitHub Actions" below for details.
...
2. By default, the frontend is deployed via GitHub Pages. If you want to continue to use GH Pages, make sure it is enabled for your project in the GH repo Settings (serving from the root of the `gh-pages` branch). This will also work with a custom domain if properly configured. If you would like to not use GitHub Pages, simply remove the "Deploy Frontend" step from the `.github/workflows/deploy.yml` file.

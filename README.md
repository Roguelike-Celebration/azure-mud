...
5. `npm run dev` will start a local copy of the client (at `http://localhost:1234/index.html`) and the server (at `http://localhost:3000`) . It auto-watches changes to all code, and attempts to live-reload any connected browser instances. If you just want to run the frontend, `npm run dev:client` will do that (or `npm run dev:server` for the backend).

6. If you are using a local server, note that trying to log in via email will not actually send you an email. When you submit the login form, the dev server's console should print out a valid URL for you to copy/paste into your browser that will log you in.

7. `npm run build` will generate a bundled version of the webapp for distribution.
